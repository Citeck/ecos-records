    ...
    fun test() {
        ...
        RequestContext.doWithCtx(services) {
            metaById.keys.forEach { recId ->
                ...
                val creator = records.getAtt(RecordRef.valueOf("test@$recId"), RecordConstants.ATT_CREATOR).asText()
                val created = records.getAtt(RecordRef.valueOf("test@$recId"), RecordConstants.ATT_CREATED).asText()
                val modifier = records.getAtt(RecordRef.valueOf("test@$recId"), RecordConstants.ATT_MODIFIER).asText()
                val modified = records.getAtt(RecordRef.valueOf("test@$recId"), RecordConstants.ATT_MODIFIED).asText()
