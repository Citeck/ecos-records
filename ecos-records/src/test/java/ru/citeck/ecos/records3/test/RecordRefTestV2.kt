...
class RecordRefTestV2 {
    ...
    fun test() {
        ...
        assertThat(ref.withDefault(appName = "app", sourceId = "src", id = "local-id")).isSameAs(ref)

        assertThat(ref.withDefault(appName = "app2")).isSameAs(ref)
        assertThat(ref.withDefault(appName = "app2", sourceId = "src2")).isSameAs(ref)
        assertThat(ref.withDefault(appName = "app2", sourceId = "src2", id = "local-id2")).isSameAs(ref)
        ...
        assertThat(emptyRef.withDefault(appName = "app", sourceId = "src", id = "local-id").toString()).isEqualTo("app/src@local-id")
    }
}
